"""Construct a local Cascade heat or finalist training dataset.

The generator loading, validation, seeding, stream budgeting, and digesting
are delegated to the Cascade subnet implementation. Use ``--stage heat`` for
the configured screening budget or ``--stage final`` for the full finalist
training budget.
"""
...
def _parser() -> argparse.ArgumentParser:
	...
	parser.add_argument("--stage", choices=("heat", "final"), required=True,
						help="Dataset stage: heat screen or full finalist training.")
	...
	parser.add_argument("--arch-preset", default=None,
						help="Final model size; only applies to --stage final.")
	...
	parser.add_argument("--overwrite", action="store_true",
						help="Remove existing shards and manifest before rebuilding.")
...
def _final_contract(cfg, arch_preset: str | None):
	...
	raise ValueError(
		f"unknown --arch-preset {arch_preset!r}; available: {', '.join(available)}"
	)


def _stage_contract(cfg, stage: str, arch_preset: str | None):
	if stage == "heat":
		if arch_preset:
			raise ValueError("--arch-preset applies only to --stage final")
	...
	return _final_contract(cfg, arch_preset)


def _clear_output(output_dir: Path, overwrite: bool) -> None:
	output_dir.mkdir(parents=True, exist_ok=True)
	if not any(output_dir.glob("shard-*.npy")) and not (output_dir / "manifest.json").exists():
		return
	if not overwrite:
...
def construct_dataset(args: argparse.Namespace) -> dict[str, object]:
	...
	contract = _stage_contract(cfg, args.stage, args.arch_preset)
	...
	_clear_output(output_dir, args.overwrite)
	...
	with open_round_stream(
		...
		manifest = {
			...
			"budget_denomination": denomination,
			"budget_hours": float(contract.target_train_hours),
